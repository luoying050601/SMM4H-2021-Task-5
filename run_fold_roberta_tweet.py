...
import torch
import random
from transformers import AdamW, AutoTokenizer, RobertaForSequenceClassification
import numpy as np
import pandas as pd
...
class config:
    ...
    SEED = 7426
    KFOLD = 5
    SAVE_DIR = Proj_dir + '/run_roberta_tweet_oof'
...
def test_prediction(kfold=None):
    ...
    if kfold is not None:
        for i in range(config.KFOLD):
            model.load_state_dict(
                torch.load(os.path.join(config.SAVE_DIR, f'model/model_rt_{i}_' + str(config.SEED) + '.bin')))
        ...
        scores['max_prob'] = (np.max([scores['prob_0'], scores['prob_1'], scores['prob_2'], scores['prob_3'], scores['prob_4']],
                                     axis=0) >= 0.5) * 1
        # average probality
        ...
        model.load_state_dict(torch.load(os.path.join(config.SAVE_DIR, f'model/model_rt_' + str(config.SEED) + '.bin')))
        y_preds = test_fn(test_data_loader, model, device)
    ...
    submission.to_csv(os.path.join(config.SAVE_DIR, 'output/submission_final.tsv'), index=False, sep='\t')
        ...
        for batch_size_t in config.BATCH_SIZE_LIST_T:
            ...
            for epoches in config.EPOCHS_LIST:
                ...
                for epoch in range(epoches):
                    ...
                    if fold is None:
                        es(valid_loss, model,
                           model_path=os.path.join(config.SAVE_DIR, f'model/model_rt_' + str(config.SEED) + '.bin'))
                    else:
                        es(valid_loss, model,
                           model_path=os.path.join(config.SAVE_DIR,
                                                   f'model/model_rt_{fold}_' + str(config.SEED) + '.bin'))
                    if es.early_stop:
                ...
                if fold is None:
                    model.load_state_dict(
                        torch.load(os.path.join(config.SAVE_DIR, 'model/model_rt_' + str(config.SEED) + '.bin')))
                else:
                    model.load_state_dict(
                        torch.load(
                            os.path.join(config.SAVE_DIR, f'model/model_rt_{fold}_' + str(config.SEED) + '.bin')))
                model.to(device)
