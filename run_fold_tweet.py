...
class config:
    ...
    # lr = 2e-5
    # SEED = 5  # 5
    SEED = random.randint(0, 100)  # 3 - 5
    KFOLD = 5
    SAVE_DIR = Proj_dir + '/run_tweet_bert_oof'
    TRAIN_FILE = '../../../data/train.tsv'
    VAL_FILE = '../../../data/valid.tsv'
    TEST_FILE = '../../../data/test.tsv'
    OOF_FILE = os.path.join(SAVE_DIR, 'output/oof'+str(SEED)+'.csv')
    MAX_LEN = 96
...
def test_prediction(kfold=None):
    ...
    if kfold is not None:
        for i in range(config.KFOLD):
            model.load_state_dict(
                torch.load(os.path.join(config.SAVE_DIR, f'model/model_{i}_' + str(config.SEED) + '.bin')))
        ...
        model.load_state_dict(torch.load(os.path.join(config.SAVE_DIR, f'model/model_' + str(config.SEED) + '.bin')))
        y_preds = test_fn(test_data_loader, model, device)
    ...
    submission.to_csv(os.path.join(config.SAVE_DIR, 'submission_final'+str(config.SEED)+'.tsv'), index=False, sep='\t')
        ...
        for batch_size_t in config.BATCH_SIZE_LIST_T:
            ...
            for epoches in config.EPOCHS_LIST:
                ...
                for epoch in range(epoches):
                    ...
                    if fold is None:
                        es(valid_loss, model,
                           model_path=os.path.join(config.SAVE_DIR, f'model/model_' + str(config.SEED) + '.bin'))
                    else:
                        es(valid_loss, model,
                           model_path=os.path.join(config.SAVE_DIR, f'model/model_{fold}_' + str(config.SEED) + '.bin'))
                    if es.early_stop:
                ...
                if fold is None:
                    model.load_state_dict(
                        torch.load(os.path.join(config.SAVE_DIR, 'model/model_' + str(config.SEED) + '.bin')))
                else:
                    model.load_state_dict(
                        torch.load(os.path.join(config.SAVE_DIR, f'model/model_{fold}_' + str(config.SEED) + '.bin')))
                model.to(device)
...
def run_fold(fold_idx):
    ...
    # df_train = pd.concat([df_0[0:1026], df_1]).sample(frac=1).reset_index()
    print(len(df_train))
    ...
    y, y_dict = run(df_train, df_val, fold_idx)
