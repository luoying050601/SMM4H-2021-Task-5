...
from src.com.util.misc import ArgParseDefault, add_bool_arg
...
def segment_sentences(text, args):
    ...
    doc = nlp(text)
    regex_fillers = r'(^\d {username}$)|^{username}$|(^\d {url}$)|^{url}$'.format(username=args.username_filler,
                                                                                  url=args.url_filler)
    num_tokens = len(doc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_data', required=False, help='Path to folder with txt files. \
    ...
    parser.add_argument('--model_class', default='bert_large_uncased_wwm',
                        help='Model class')
    parser.add_argument('--username_filler', default='twitteruser', type=str, help='Username filler')
    parser.add_argument('--url_filler', default='twitterurl', type=str,
    ...
    add_bool_arg(parser, 'replace_multiple_usernames', default=True,
                 help='Replace "@user @user" with "2 <username_filler>"')
    add_bool_arg(parser, 'replace_multiple_urls', default=True,
                 help='Replace "http://... http://.." with "2 <url_filler>"')
    add_bool_arg(parser, 'remove_unicode_symbols', default=True,
                 help='After preprocessing remove characters which belong to unicode category "So"')
    add_bool_arg(parser, 'remove_accented_characters', default=False,
    ...
    # ----------------------------------------------------------------------------------------------
    text = "@_faschaniecesta My school just bought us back from abroad because of the Coronavirus 😭😭🙃"
    text_clean = preprocess_bert(text,args)
    print(text_clean)
